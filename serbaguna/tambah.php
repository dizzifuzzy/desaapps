                                ...
                                <h4 class="mt-0 header-title">Tambah Data</h4>
                                    ...
                                    if($_POST['nomorsurat']){
                                        ...
                                        if(($nosurat==NULL)||($tanggalsurat==NULL)||($id_dd==NULL)||($alamat==NULL)||($desa==NULL)||($kecamatan==NULL))
                                        {
                                            ...
                                            $cek_nomor_surat = $mysqli->query("SELECT * FROM serba_guna WHERE no_surat_serbaguna ='".$nosurat."'");
                                            if($cek_nomor_surat->fetch_array()){
                                                echo '<div class="alert alert-danger">Nomor Surat tidak boleh sama!</div>';
                                            }else{
                                                $submit = $mysqli->query("INSERT INTO serba_guna (no_surat_serbaguna, tgl_surat_serbaguna, id_dd, pendidikan_serbaguna, pekerjaan_serbaguna, perkawinan_serbaguna, alamat_serbaguna, desa_serbaguna, kecamatan_serbaguna, tgl_serbaguna, status_serbaguna, id_petugas) VALUES ('$nosurat', '$tanggalsurat', '$id_dd', '$pendidikan', '$pekerjaan', '$perkawinan', '$alamat', '$desa', '$kecamatan', '$tgl','1', '$idpetugas');");
                                                $idnya = $mysqli->insert_id;
                                                $mysqli->query("INSERT INTO aktivitas (kunci_aktivitas, nama_aktivitas,waktu_aktivitas,id_petugas) VALUES ('id: $idnya - serbaguna','Menambahkan data Serba Guna dengan nomor surat $nosurat','$tgl','$idpetugas')");
                                                if($submit){
