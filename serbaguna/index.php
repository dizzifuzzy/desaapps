...
$cekserbaguna = $mysqli->query("SELECT * FROM serba_guna JOIN data_diri ON serba_guna.id_dd = data_diri.id_dd where serba_guna.status_serbaguna=1");
...
<?php include "../header.php";?>
                    ...
                    <div class="col-12">
                        <div class="card m-b-30">
                            <div class="card-body">
                                ...
                                <h4 class="mt-0 header-title">Daftar Serba Guna</h4>
                                <p class="text-muted m-b-30 font-14">Jumlah data yang terdaftar di serba guna.
                                </p>
                                ...
                                <table id="datatable" class="table table-bordered">
                                    ...
                                    <tbody>
                                    <?php $no = 0; while($row=$cekserbaguna->fetch_assoc()){  $no++;?>
                                    <tr>
                                        <td><?=$no;?></td>
                                        <td><?=$row['no_surat_serbaguna'];?></td>
                                        <td><?=$row['tgl_surat_serbaguna'];?></td>
                                        <td><?=$row['nik_dd'];?> - <?=$row['nama_dd'];?></td>
                                        <td><a href="detail.php?id=<?=$row['id_serbaguna'];?>">Detail</a>/<a href="ubah.php?id=<?=$row['id_serbaguna'];?>">Ubah</a>/<a href="?hapus=<?=$row['id_serbaguna'];?>">Hapus</a></td>
                                    </tr>
                                    <?php } ?>
                                    </tbody>
                                </table>
    ...
    if($_GET['hapus']){
        $hapus = $_GET['hapus'];
        $mysqli->query("UPDATE serba_guna SET status_serbaguna=0 WHERE id_serbaguna='$hapus'");
        echo '
        <script>
