...
$ceksku = $mysqli->query("SELECT * FROM sku JOIN data_diri ON sku.id_dd = data_diri.id_dd where sku.status_sku=1");
...
<?php include "../header.php";?>
                    ...
                    <div class="col-12">
                        <div class="card m-b-30">
                            <div class="card-body">
                                ...
                                <h4 class="mt-0 header-title">Daftar Surat Keterangan Usaha</h4>
                                <p class="text-muted m-b-30 font-14">Jumlah data yang terdaftar di SKU.
                                </p>
                                ...
                                <table id="datatable" class="table table-bordered">
                                    ...
                                    <tbody>
                                    <?php $no = 0; while($row=$ceksku->fetch_assoc()){  $no++;?>
                                    <tr>
                                        <td><?=$no;?></td>
                                        <td><?=$row['no_surat_sku'];?></td>
                                        <td><?=$row['tgl_surat_sku'];?></td>
                                        <td><?=$row['nik_dd'];?> - <?=$row['nama_dd'];?></td>
                                        <td><a href="detail.php?id=<?=$row['id_sku'];?>">Detail</a>/<a href="ubah.php?id=<?=$row['id_sku'];?>">Ubah</a>/<a href="?hapus=<?=$row['id_sku'];?>">Hapus</a></td>
                                    </tr>
                                    <?php } ?>
                                    </tbody>
                                </table>
    ...
    if($_GET['hapus']){
        $hapus = $_GET['hapus'];
        $mysqli->query("UPDATE sku SET status_sku=0 WHERE id_sku='$hapus'");
        echo '
        <script>
